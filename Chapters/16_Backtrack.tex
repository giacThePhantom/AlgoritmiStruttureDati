...
Sia $S$ il vettore contenente le soluzioni parziali $S[1\dots i]$ con $i$ l'indice corrente, $C$ l'insieme dei possibili candidati per estendere la 
...
\caption{\protect\Boolean \protect\Enumeration{$<$dati problema$<$, \protect\Item[] S, \protect\Int i,$<$dati parziali$>$}}
...
\caption{\protect\Boolean \protect\SubsetRec{\protect\Int n, \protect\Item[] S, \protect\Int i}}
...
\caption{\protect\Boolean \protect\Subset{\protect\Int n}}
...
\caption{\protect\Boolean \protect\ProcessSolution{\protect\Int[] S, \protect\Int n}}
\Print ``$\{$"\;
\For{\Int i = 1 \To n}{
	\If{S[i]}{
		\Print i, `` "\;
...
\caption{\protect\Boolean \protect\PermRec{\protect\Int[] A, \protect\Int n, \protect\Int missing, \protect\Item[] S, \protect\Int i}}
\uIf{missing == 0}{
...
\ForEach{c $\in$ C}{
	...
	\ElseIf{\Cavallo{S, i + 1, x + $m_x[c]$, y + $m_y[c]$}}{
		\Return \True\;
	}
	S[x][y] = 0\;
}
...
\caption{\protect\Set \protect\Choices{\protect\Int[][] S, \protect\Int i, \protect\Int x}}
\Set C = \SetCos{}\;
\For{\Int i = 1 \To 8}{
	$n_x = x + m_x[i]$\;
	$n_y = y + m_y[i]$\;
